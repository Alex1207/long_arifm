...
namespace MathFuncs
{
	...
	bool LongInt::Read(char *flag, char *fname)
	{
		...
		for (int i=0; i<=size; i++)
			...
			{
				printf("There are strange symbols in the file %s... ", fname);
				return false;
			}
		if (size==0)
		{
	...
	LongInt LongInt::Sum(LongInt a, LongInt b, LongInt &c)
	{
		...
		{
			c.digits[i] = a.digits[i] + b.digits[i] + v; // суммируем последние разряды чисел
			v = (c.digits[i] / 10); // если есть разряд для переноса, переносим его в следующий разряд
			c.digits[i] %= 10; // если есть разряд для переноса он отсекается
		}
		for (int i=mins+1; (i <= maxs); i++)//&&(v!=0)
		{
			if(mins==a.size)
	...
	LongInt LongInt::Sub(LongInt a, LongInt b, LongInt &c)
	{
		...
		{	
			if (i<=b.size)
				...
				c1=a.digits[i]-loan;
			if(c1>=0)	//если нет заема, 
			{
				c.digits[i]=c1;	//сохраняем
				loan=0;
			}
	...
	LongInt LongInt::Mul(LongInt a, LongInt b, LongInt &c)
	{
		...
		c.DelNull();//DelNull(c, sizeC);//избавляемся от нулей
			return c;
	}
	LongInt LongInt::operator*(const LongInt& a)
	{
	...
	void LongInt::Div(LongInt a, LongInt b, LongInt &q, LongInt &r, LongInt c, LongInt m)
	{
		...
		r.DelNull();//DelNull(r, sizer);
		for (int i=b.size; i>0; i--)
			m.digits[i]=b.digits[i];
		m.size=b.size;//long sizem=sizeB;
		m.DelNull();
	...
	LongInt LongInt::Pow (LongInt a, LongInt b, LongInt mod)
	{
		...
		for (int j=d.size-1; j>=1; j--)
		{
			...
			if (a0.size>=mod.size)
			{
				...
				q.size=1;//long sizer=sizeC, sizeq=1;
	...
	bool LongInt::CheckMinus()
	{
		return minus;
	}
	bool LongInt::CheckSize()
	{
		if (size<0)
			return true;
		return false;
